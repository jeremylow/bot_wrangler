...
class API(object):
    ...
    def update_with_media(self, filename, *args, **kwargs):
        ...
        return bind_api(
            ...
            payload_type='status',
            allowed_param=[
                'status', 'possibly_sensitive', 'in_reply_to_status_id', 'lat',
    ...
    def video_upload(self, filename, *args, **kwargs):
        ...
        headers, post_data, fp = API._chunk_video(command='init',
                                                  filename=filename,
                                                  form_field='media',
                                                  f=f)
        kwargs.update({'headers': headers, 'post_data': post_data})
    ...
    def _chunk_video(command,
        ...
        if command == 'init':
            ...
            body.append(b'')
            body.append(bytes(str(media_id).encode('utf-8')))
            body.append(b'--' + BOUNDARY)
            body.append(b'Content-Disposition: form-data; name="segment_index"')
            body.append(b'')
            body.append(bytes(str(segment_index).encode('utf-8')))
            body.append(b'--' + BOUNDARY)
            body.append(
                bytes('Content-Disposition: form-data; name="{0}"; filename="{1}"'.format(
                    form_field,
        ...
        # [print(type(line), line) for line in body]
        body = b'\r\n'.join(body)
        # print(body)
        # build headers
